...
__global__ void diffusion_kernel_shared5(REAL *f1, REAL *f2,
...
void Diffusion3DCUDAShared5::RunKernel(int count) {
  int flag = 0;
  ...
  dim3 grid_dim(nx_ / bdimx, ny_ / bdimy, 4);
  ...
  #pragma omp parallel num_threads(2) shared(flag)
  {
    ...
    else
    {
      ...
      for (int i = 0; i < count; i+=2) {
        diffusion_kernel_shared5<<<grid_dim, block_dim,
...
void Diffusion3DCUDAShared5::InitializeBenchmark() {
  Diffusion3DCUDA::InitializeBenchmark();
  CUDA_SAFE_CALL(cudaFuncSetCacheConfig(diffusion_kernel_shared5,
